...
import { RefreshCw, Search, XCircle, ArrowRight, Pencil } from 'lucide-react';
...
const RidesTable: React.FC = () => {
  ...
  useEffect(() => { fetchRides(debouncedSearch); }, [debouncedSearch, fetchRides]);
    ...
    <div className="animate-fade-in">
      <div className="flex items-center gap-3 mb-5">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search by route..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <button onClick={() => fetchRides(debouncedSearch)}
      ...
      {loading ? (
        ...
        <>
          <div className="overflow-x-auto rounded-xl border border-gray-200 shadow-sm">
            <table className="min-w-full bg-white text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-50">
                {rides.length === 0 ? (
                  <tr><td colSpan={7} className="px-6 py-10 text-center text-sm text-gray-400">{search ? 'No rides match your search.' : 'No rides found.'}</td></tr>
                ) : rides.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">
                      ...
                      {r.vehicle_color && <span className="block text-xs text-gray-400">{r.vehicle_color}</span>}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600">
                      {r.departure_time ? (
                    ...
                    <td className="px-6 py-4 text-sm font-bold text-indigo-600">₦{r.price_per_seat?.toLocaleString() || '—'}</td>
                    <td className="px-6 py-4 text-sm text-gray-700">
                      {r.available_seats != null && r.total_seats != null ? `${r.total_seats - r.available_seats}/${r.total_seats}` : '—'}
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold capitalize ${statusStyle(r.status)}`}>{r.status}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => setEditingRide(r)} title="Edit ride"
                          className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-100 rounded-lg transition-colors">
                          <Pencil size={13} /> Edit
                        </button>
                        {r.status === 'open' && (
                          <button onClick={() => cancelRide(r.id)} title="Cancel ride"
          ...
          <div className="mt-3 text-xs text-gray-400">Showing {rides.length} ride{rides.length !== 1 ? 's' : ''}</div>
        </>
      )}
      ...
      {editingRide && (
        <EditRideModal
          ...
          onSuccess={() => { setEditingRide(null); fetchRides(debouncedSearch); }}
        />
      )}
    </div>
  );
