...
router.post('/', async (req, res) => {
    try {
        ...
        if (conflictingBookings?.length)
            return res.status(409).json({ error: 'You have already booked a seat on this exact ride.' });
        ...
        if (paymentMethod === 'wallet') {
            ...
            await NotificationService.sendNotification(ride.driver_id, 'New Ride Booking', `${passengerName} booked ${seatsBooked} seat(s) for your ride from ${ride.from} to ${ride.to}.`, 'booking', { bookingId: booking.id, rideId });
            await NotificationService.notifyAdmins('New Ride Booking', `A new booking was made for ride ${rideId} by user ${req.userId}.`, 'admin_alert', { bookingId: booking.id, rideId });
...
router.post('/:id/accept', async (req, res) => {
    try {
        ...
        if (!isDriver)
            return res.status(403).json({ error: 'Only the driver can accept this booking' });
...
router.get('/', async (req, res) => {
    try {
        ...
        let query = supabase
            .from('bookings')
            .select('*, rides!inner(from, to, departure_time, price_per_seat, driver_id, driver:driver_id(first_name, last_name, profile_picture, ratings))')
            .order('created_at', { ascending: false });
        if (role === 'driver')
            query = query.eq('rides.driver_id', req.userId);
        else
            query = query.eq('rider_id', req.userId);
...
router.post('/:id/cancel', async (req, res) => {
    try {
        ...
        if (booking.payment_status === 'completed') {
            ...
            if (wallet)
                await supabase.from('wallets').update({ balance: wallet.balance + booking.total_price, held_amount: wallet.held_amount - booking.total_price }).eq('user_id', req.userId);
        }
