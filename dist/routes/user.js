...
router.get('/activity', async (req, res) => {
    try {
        const [{ count: totalBookings, error: bError }, { count: completedBookings, error: cbError }, { count: totalRides, error: rError }, { count: activeDriverRides, error: aError }, { data: profile, error: pError },] = await Promise.all([
        ...
        return res.json({
            ...
            averageRating: profile?.ratings ?? 5.0,
...
router.post('/fcm-token', async (req, res) => {
    try {
        ...
        const { error } = await supabase
            ...
            .upsert({ user_id: req.userId, token, device_type: deviceType, updated_at: new Date().toISOString() }, { onConflict: 'user_id, token' });
        if (error)
            throw error;
