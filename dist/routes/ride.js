...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (ride.status !== 'open')
            return res.status(400).json({ error: 'Only open rides can be edited' });
        // Update the ride
...
router.get('/search', async (req, res) => {
    try {
        const { from, to, date, limit = 50, pickupLat, pickupLng, pickupRadius = 10, dropoffLat, dropoffLng, dropoffRadius = 10 } = req.query;
        ...
        let query = supabase
            ...
            .eq('status', 'open');
        if (from)
            query = query.ilike('from', `%${from}%`);
        if (to)
            query = query.ilike('to', `%${to}%`);
        if (date) {
...
router.get('/driver', async (req, res) => {
    try {
        ...
        if (error)
            return res.status(500).json({ error: 'Failed to fetch your rides' });
    ...
    try {
        ...
        if (bookings?.length) {
            for (const b of bookings) {
                ...
                if (wallet)
                    await supabase.from('wallets').update({ balance: wallet.balance + b.total_price, held_amount: wallet.held_amount - b.total_price }).eq('user_id', b.rider_id);
            }
        }
...
router.post('/:id/complete', async (req, res) => {
    try {
        ...
        if (ride.driver_id !== req.userId)
            return res.status(403).json({ error: 'Unauthorized' });
        if (ride.status !== 'open')
            return res.status(400).json({ error: 'Ride is not open' });
...
router.post('/:id/rate', async (req, res) => {
    try {
        ...
        if (bookingError || !booking)
            return res.status(403).json({ error: 'You did not complete this ride as a passenger' });
        ...
        const { data: existingReview } = await supabase
            ...
            .single();
        if (existingReview)
            return res.status(400).json({ error: 'You have already rated this driver for this ride' });
